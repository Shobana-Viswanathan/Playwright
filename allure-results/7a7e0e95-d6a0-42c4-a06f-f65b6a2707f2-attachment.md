...
- Name: testmuaipromptalert.test.ts >> Confirm Alert
- Location: tests\testmuaipromptalert.test.ts:2:1
...
Expected substring: "You have entered"
Received string:    ""
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('#confirm-demo')
    7 × locator resolved to <p class="mt-20" id="confirm-demo"></p>
      - unexpected value ""
...
```ts
  ...
  5  |         const text=alert.defaultValue();
  6  |         console.log(text);
  7  |         await alert.accept("Shobi");
  8  |     });
  9  |     await page.locator('//button[text()="Click Me"]').nth(2).click();
> 10 |     await expect (page.locator('#confirm-demo')).toContainText('You have entered');
     |                                                  ^ Error: expect(locator).toContainText(expected) failed
  11 | 
  12 | })
```