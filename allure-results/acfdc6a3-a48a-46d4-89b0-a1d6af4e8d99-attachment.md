...
- Name: testonlyprac.test.ts >> Login test 1
- Location: tests\testonlyprac.test.ts:2:6
...
```ts
  ...
  2  | test.only('Login test 1',async({page})=>{
...
> 9  |     await expect.soft(welcometxt).toBe('Welcome Shobs');
  ...
  10 |     
  11 | })
  12 | test('Search',async({page})=>{
  13 |     console.log("Search");
  14 | })
  15 | test('Logout',async({page})=>{
  16 |     console.log("Logout");
  17 | })
```