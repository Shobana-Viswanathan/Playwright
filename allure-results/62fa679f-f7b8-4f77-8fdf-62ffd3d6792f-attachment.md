...
> 10 |     await expect.soft(welcometxt).toBe('Welcome Shobs');
     |                                   ^ Error: expect(received).toBe(expected) // Object.is equality
  11 | })
  12 | test.skip("Invalid Login Test 2 @Invalid",async({page})=>{
  13 |     test.skip(true,"Under maintainance");
  14 |      await page.goto("https://www.demoblaze.com/");
  15 |     await page.click('#login2')
  16 |     await page.fill('#loginusername',"Shobs");
  17 |     await page.fill('#loginpassword',"shobi11");
  18 |     await page.click('button[onclick="logIn()"]');
  19 |     await expect(page.getByRole("link",{name:"Log out"})).toBeVisible();
  20 |     const welcometxt = page.locator("#nameofuser").textContent();
  21 |     expect(welcometxt).toBe('Welcome Shobs');
  22 | 
  23 | })
```