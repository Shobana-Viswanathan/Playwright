...
- Name: testmuai.test.ts >> Testmuai
- Location: tests\testmuai.test.ts:2:1
...
Call log:
  ...
  - waiting for locator('//p[text()="Shobi"]')
...
```ts
  ...
  2  | test('Testmuai',async({page})=>{
  3  |     await page.goto("https://www.testmuai.com/selenium-playground/simple-form-demo/");
  4  |     const title=await page.title();
  5  |     console.log(title);
  6  |     const input= page.locator("//input[@id='user-message']").getByText("Please enter your Message");
  7  |     console.log('Before value:',input);
  8  |     await page.locator("//input[@id='user-message']").fill("Shobi");
  9  |     await page.click('//button[@id="showInput"]')
> 10 |     await expect (page.locator('//p[text()="Shobi"]')).toBeVisible();
     |                                                        ^ Error: expect(locator).toBeVisible() failed
  11 | 
  12 | 
  13 | })
