...
> 9  |     await expect.soft(welcometxt).toBe('Welcome Shobs');
     |                                   ^ Error: expect(received).toBe(expected) // Object.is equality
  10 | })
  11 | test.skip("Login Test 2",async({page})=>{
  12 |      await page.goto("https://www.demoblaze.com/");
  13 |     await page.click('#login2')
  14 |     await page.fill('#loginusername',"Shobs");
  ...
  18 |     const welcometxt = page.locator("#nameofuser").textContent();
  19 |     expect(welcometxt).toBe('Welcome Shobs');
  20 | 
  21 | })
```